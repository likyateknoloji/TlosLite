...
public class RestrictedDailyIterator implements ScheduleIterator {
	...
	public RestrictedDailyIterator(int hourOfDay, int minute, int second, int[] days) {
		this(hourOfDay, minute, second, days, new Date(), false);
	}
	
	public RestrictedDailyIterator(int hourOfDay, int minute, int second, int[] days, boolean scenarioTimeAnomaly) {
		this(hourOfDay, minute, second, days, new Date(), scenarioTimeAnomaly);
	}
	
	private RestrictedDailyIterator(int hourOfDay, int minute, int second, int[] days, Date date, boolean scenarioTimeAnomaly) {
		...
		if (!calendar.getTime().before(date)  && !scenarioTimeAnomaly) {
